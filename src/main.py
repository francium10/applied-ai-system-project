...
=========================================
Demonstrates all four optional extensions from Module 3:
...
For the full VibeFinder 2.0 pipeline (RAG + agentic critic + guardrails), use:
...
BAR_FULL = 14   # characters in ASCII score bar

# Keep in sync with DIVERSITY_PENALTY in recommender.py
DIVERSITY_PENALTY_LABEL = 0.6
...
def print_standard_table(profile_name: str, prefs: dict, songs: list) -> None:
    """Challenge 4: visual ASCII table for a single profile in BALANCED mode."""
...
def print_mode_comparison(prefs: dict, songs: list) -> None:
    """Challenge 2: show how top-3 changes across all four scoring modes."""
    print_section(
        "Scoring Mode Comparison  [Challenge 2]",
        "Same profile (pop_fan) -- four different ranking strategies",
    )
    header = f"  {'Mode':<14}" + \
...
def print_diversity_comparison(prefs: dict, songs: list) -> None:
    ...
    print(table_divider())
    print(
        f"  Penalty: repeated artist score x{DIVERSITY_PENALTY_LABEL} (40% reduction)")
    print("  Result:  second LoRoom song drops, more artist variety surfaces.")


def print_extended_features(prefs: dict, songs: list) -> None:
    """Challenge 1: show popularity, era, and mood tag scoring contributions."""
    print_section(
        "Extended Features  [Challenge 1]",
        "pop_fan with mood_tags=[euphoric, bright] | preferred_decade=2020s",
    )
    results = recommend_songs(prefs, songs, k=5)
    header = (
        f"  {'#':<3}  {'Title':<24}  {'Tags on song':<30}"
        f"  {'Era':<6}  {'Pop':>3}  Score"
    )
    print(header)
    print(table_divider())
    for rank, (song, score, _explanation) in enumerate(results, 1):
        tags = song.get("mood_tags", "")[:28]
        era = song.get("release_decade", "?")
        pop = int(song.get("popularity", 0))
        print(
            f"  {rank:<3}  {cell(song['title'], 24)}  {tags:<30}"
            f"  {era:<6}  {pop:>3}  {score:.2f}"
        )
    print(table_divider())
    print(
        f"  Popularity bonus : up to +{WEIGHTS['popularity']:.2f} (normalised 0-100 to 0-1)")
    print(
        f"  Era match bonus  : +{WEIGHTS['decade']:.2f} when release_decade == preferred_decade")
    print(
        f"  Mood tag bonus   : up to +{WEIGHTS['mood_tags']:.2f} based on overlap ratio")


# ── Main ──────────────────────────────────────────────────────────────────────

def main() -> None:
    """Load catalog and run all four Module 3 challenge demonstrations."""

    # Graceful error if run from the wrong directory
    try:
        songs = load_songs("data/songs.csv")
    except FileNotFoundError:
        print("Error: data/songs.csv not found.")
    ...
    print()
    print(f"{'=' * TABLE_WIDTH}")
    print(f"  VIBEFINDER 2.0 -- Music Recommender Simulation (Module 3 CLI)")
    print(
        f"  Catalog : {len(songs)} songs  |  Max score (BALANCED): {MAX_POSSIBLE_SCORE:.1f}")
    print(
        f"  Weights : genre x{WEIGHTS['genre']} | mood x{WEIGHTS['mood']} | "
        f"energy x{WEIGHTS['energy']} | mood_tags x{WEIGHTS['mood_tags']} | "
        f"popularity x{WEIGHTS['popularity']}"
    )
    print(f"  Tip     : full AI pipeline -> python -m src.agent.planner --query '...'")
    print(f"{'=' * TABLE_WIDTH}")

    # Challenge 4 -- ASCII table for every standard profile
