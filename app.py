...
.result-rank { font-family: 'Space Mono', monospace; font-size: 0.7rem; color: #4b4b6b; }
.result-title { font-size: 1.15rem; font-weight: 600; color: #ffffff; margin: 0.15rem 0; }
.result-artist { font-size: 0.85rem; color: #6366f1; }
...
.reason-pill:hover { background: rgba(99,102,241,0.16); }
...
with st.sidebar:
    ...
    if input_mode == "🎤  Natural language":
        ...
        decade = st.selectbox("Preferred era (optional)",
                              ["", "1980s", "1990s", "2000s", "2010s", "2020s"])
        tags_raw = st.text_input("Mood tags (comma-separated)",
    ...
    diversity_penalty = st.toggle("Diversity penalty", value=True,
                                  help="Reduces score if artist already in top results")
    st.divider()
...
with tab_results:
    if not run_btn:
        ...
        # ── Guard: something must be provided ────────────────────────────────
        if not user_input:
            st.error("Please enter a query or build a profile before searching.")
            st.stop()
        ...
        if isinstance(user_input, str):
            ...
            parts = ([structured_profile.get("genre", ""),
                     ...
                     structured_profile.get("preferred_mood_tags", []))
        ...
        if critique_result.should_rerank:
            ...
            critique_result = critique(query_text, profile, results,
                                       top_similarity, attempt=2)
        ...
        log_decision(
            ...
            top_results=[{"id": s.get("id"), "title": s.get("title"),
                          "score": round(sc, 2)} for s, sc, _ in results],
        ...
        conf_class = {"HIGH": "conf-high", "MEDIUM": "conf-medium",
                      "LOW": "conf-low"}[conf_label]
        ...
        with col2:
            ...
            <div class="card" style="padding:1rem;text-align:center">
                ...
                <div class="result-title" style="font-size:0.95rem">{html_lib.escape(scoring_mode_label)}</div>
        ...
        with col3:
            ...
            <div class="card" style="padding:1rem;text-align:center">
                ...
                <div class="result-meta">{"strong" if top_similarity > 0.4 else "weak"} match</div>
            </div>""", unsafe_allow_html=True)
        ...
        for rank, (song, score, explanation) in enumerate(results, 1):
            ...
            mood_tags = html_lib.escape(song.get("mood_tags", "").replace("|", " · "))
            ...
            <div class="result-card">
                ...
                <div class="result-meta">
                    {html_lib.escape(song['genre'])} &nbsp;·&nbsp; {html_lib.escape(song['mood'])} &nbsp;·&nbsp;
...
# ── Tab 2: How it works ───────────────────────────────────────────────────────
with tab_how:
    st.markdown('<div class="section-label" style="margin-top:0.5rem">System pipeline</div>',
                unsafe_allow_html=True)
    ...
    steps = [
        ("1 · Guardrails", "#4b5563",
         ...
         "abort the pipeline; soft warnings are logged and the pipeline continues."),
        ("2 · RAG Retriever", "#6366f1",
         "Your query is converted into a feature vector using a TF-IDF bag-of-words "
         "approach over genre, mood, and tag tokens. The vector store finds the 15 most "
         "semantically similar songs by cosine similarity — this is the Retrieval-Augmented "
         "Generation step."),
        ("3 · Scoring Engine", "#8b5cf6",
         "Each candidate is scored against your taste profile using nine weighted features: "
         "genre (×3.0), mood (×2.0), energy proximity (×2.0), acousticness (×1.0), "
         "mood tags (×1.0), valence (×1.0), danceability (×0.5), popularity (×0.5), "
         "and era match (×0.5). Songs rank descending by total score."),
        ("4 · Agentic Critic", "#10b981",
         "The critic evaluates the top-k results and assigns a confidence score (0–1) "
         "using three signals: genre/mood coverage, score spread, and RAG similarity. "
         "If confidence < 0.55, it triggers a re-rank with a different scoring mode. "
         "This is the agentic loop — the system checks its own output."),
        ("5 · Logger", "#f59e0b",
         "Every run writes a structured JSON record to logs/decisions.json: timestamp, "
         "query, parsed profile, RAG candidates, top results, confidence, re-rank flag, "
         "and critic note. Full audit trail."),
    ]
    ...
    st.markdown('<div class="section-label">Scoring weights (BALANCED mode)</div>',
                unsafe_allow_html=True)
        ...
        <div style="display:flex;align-items:center;gap:1rem;margin-bottom:0.5rem">
            ...
            <div style="flex:1;background:#1a1a30;border-radius:4px;height:8px;overflow:hidden">
                <div style="width:{pct:.0f}%;height:100%;background:{color};
                            border-radius:4px;animation:bar-grow 0.6s cubic-bezier(0.22,1,0.36,1) both">
                </div>
            </div>
...
with tab_log:
    ...
    if not records:
        ...
        for rec in reversed(records):
            ...
            rerank  = "↻ re-ranked" if rec.get("rerank_triggered") else "first pass"
            ...
            top_str = html_lib.escape(
                ", ".join(r.get("title", "?") for r in rec.get("top_results", [])[:3])
            )
            ...
            <div class="log-entry">
                <div style="display:flex;justify-content:space-between;margin-bottom:0.4rem">
                    <span>{ts}</span>
                    <span style="color:{col}">conf {conf:.2f} · {rerank}</span>
                </div>
                <div><span>query:</span> {query}</div>
                <div><span>mode:</span> {mode} &nbsp;·&nbsp;
                     <span>top songs:</span> {top_str}</div>
            </div>""", unsafe_allow_html=True)
